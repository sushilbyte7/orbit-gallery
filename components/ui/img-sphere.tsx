...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
    ...
    const velocityRef = useRef<VelocityState>({ x: 0, y: autoRotateSpeed });
    ...
    const actualContainerSize = responsiveDimensions.containerSize;
    const actualSphereRadius = responsiveDimensions.sphereRadius || actualContainerSize * 0.5;
    const baseImageSize = actualContainerSize * responsiveDimensions.baseImageScale;
    ...
    const updateMomentum = useCallback((dt = 1) => {
        if (isDragging) return;

        const vel = velocityRef.current;

        // X: decay toward 0 (frame-rate independent)
        vel.x = vel.x * Math.pow(momentumDecay, dt);

        // Y: lerp back toward natural autoRotateSpeed (frame-rate independent)
        const targetY = autoRotate ? autoRotateSpeed : 0;
        vel.y = vel.y + (targetY - vel.y) * (0.025 * dt);

        if (!autoRotate && Math.abs(vel.x) < 0.01 && Math.abs(vel.y) < 0.01) {
            vel.x = 0;
            vel.y = 0;
        }

        setRotation(prev => {
            const rawX = prev.x + clampRotationSpeed(vel.x);
            const clampedX = Math.max(-70, Math.min(70, rawX));
            // Frame-rate independent restore toward level
            const restoredX = clampedX * Math.pow(0.985, dt);

            return {
                x: restoredX,
                y: SPHERE_MATH.normalizeAngle(prev.y + clampRotationSpeed(vel.y)),
                z: prev.z
            };
        });
    ...
    useEffect(() => {
        const calculateResponsiveDimensions = () => {
            ...
            let newSphereRadius = sphereRadius;
            let newBaseImageScale = baseImageScale;

            if (isMobile) {
                // Mobile responsive calculations
                const mobilePadding = 24; // Reduced padding for more space

                if (isSmallMobile) {
                    // Extra small devices (320px–480px)
                    newContainerSize = viewportWidth * 0.94;
                    newSphereRadius = newContainerSize * 0.47;

                    // 🔥 FIX
                    newBaseImageScale = 0.22;
                } else {
                    // Tablets and medium mobile (480px–768px)
                    newContainerSize = viewportWidth * 0.88;
                    newSphereRadius = newContainerSize * 0.48;

                    // 🔥 FIX
                    newBaseImageScale = 0.20;
                }
            ...
            setResponsiveDimensions({
                ...
                baseImageScale: newBaseImageScale
            });
        };
    ...
    }, [containerSize, sphereRadius, baseImageScale]);
    ...
    useEffect(() => {
        const animate = () => {
            ...
            const dt = (now - lastFrameTime.current) / 16.666;
    ...
    const renderSpotlightModal = () => {
        ...
        return (
            <div
                className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8 bg-black/40 backdrop-blur-sm"
            ...
            >
                <div
                    className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl"
                ...
                >
                    <div className="relative aspect-square">
                        ...
                        <button
                            onClick={() => setSelectedImage(null)}
                            className="absolute top-3 right-3 w-9 h-9 bg-white/10 backdrop-blur-md rounded-full text-white flex items-center justify-center hover:bg-white/20 transition-all cursor-pointer"
                        >
                            <X size={18} />
                        </button>
                    </div>
    ...
    if (!isMounted) {
        return (
            <div
                ...
                style={{ width: actualContainerSize, height: actualContainerSize, maxWidth: '100%' }}
            >
        ...
        return (
            ...
            >
                <div className="text-gray-400 text-center px-4">
                    ...
                    <p className="text-xs sm:text-sm">Add images to the images prop</p>
                </div>
            </div>
        );
    }
